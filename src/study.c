...
int main() {
    char path_of_file[500] = "obj/Mercedes_Benz.obj";
    data_t some_data;
    ...
    // printf("%d ", some_data.polygons->numbers_of_vertexes_in_facets);
    //  for (int i = 0; i < some_data.count_of_polygons+1; i++)
    //  {
    //      printf("%d ", some_data.polygons[i].numbers_of_vertexes_in_facets);
    //  }
    for (int i = 0; i < some_data.count_of_polygons + 1; i++) {
